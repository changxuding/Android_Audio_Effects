...
int EffectRelease(effect_handle_t handle){
    ...
    printf("\tEffectRelease %p", handle);
    if (pContext == NULL){
        printf("\tLVM_ERROR : EffectRelease called with NULL pointer");
        return -EINVAL;
    }
    ...
    for (int i=0; i<LVM_NR_MEMORY_REGIONS; i++){
        if (MemTab.Region[i].Size != 0){
            if (MemTab.Region[i].pBaseAddress != NULL){
                ...
                printf("\tLVM_ERROR : free() - trying to free with NULL pointer %" PRIu32 " bytes "
...
int Reverb_init(ReverbContext *pContext){
    ...
    for (int i=0; i<LVM_NR_MEMORY_REGIONS; i++){
        if (MemTab.Region[i].Size != 0){
            ...
            if (MemTab.Region[i].pBaseAddress == LVM_NULL){
                printf("\tLVREV_ERROR :Reverb_init CreateInstance Failed to allocate %" PRIu32
                        " bytes for region %u\n", MemTab.Region[i].Size, i );
                bMallocFailure = LVM_TRUE;
            }else{
                printf("\tReverb_init CreateInstance allocate %" PRIu32
    ...
    if(bMallocFailure == LVM_TRUE){
        for (int i=0; i<LVM_NR_MEMORY_REGIONS; i++){
            if (MemTab.Region[i].pBaseAddress == LVM_NULL){
                printf("\tLVM_ERROR :Reverb_init CreateInstance Failed to allocate %" PRIu32
                        " bytes for region %u - Not freeing\n", MemTab.Region[i].Size, i );
            }else{
                printf("\tLVM_ERROR :Reverb_init CreateInstance Failed: but allocated %" PRIu32
...
int Reverb_command(effect_handle_t  self,
    ...
    if (pContext == NULL){
        printf("\tLVM_ERROR : Reverb_command ERROR pContext == NULL");
        return -EINVAL;
    }

    //printf("\tReverb_command INPUTS are: command %d cmdSize %d",cmdCode, cmdSize);

    switch (cmdCode){
        case EFFECT_CMD_INIT:
            //printf("\tReverb_command cmdCode Case: "
            //        "EFFECT_CMD_INIT start");

            if (pReplyData == NULL || replySize == NULL || *replySize != sizeof(int)){
                printf("\tLVM_ERROR : Reverb_command cmdCode Case: "
        ...
        case EFFECT_CMD_GET_PARAM:{
            ...
            *replySize = sizeof(effect_param_t) + voffset + p->vsize;
            ...
            //printf("\tReverb_command EFFECT_CMD_GET_PARAM "
        ...
        case EFFECT_CMD_SET_PARAM:{
            ...
            //printf("\tReverb_command EFFECT_CMD_SET_PARAM param %d, *replySize %d, value %d ",
            //        *(int32_t *)((char *)pCmdData + sizeof(effect_param_t)),
            //        *replySize,
            ...
            if (p->psize != sizeof(int32_t)){
                printf("\t4LVM_ERROR : Reverb_command cmdCode Case: "
                        "EFFECT_CMD_SET_PARAM: ERROR, psize is not sizeof(int32_t)");
                return -EINVAL;
            }

            //printf("\tn5Reverb_command cmdSize is %d\n"
        ...
        case EFFECT_CMD_ENABLE:
            ...
            *(int *)pReplyData = 0;
            ...
            //printf("\tEFFECT_CMD_ENABLE SamplesToExitCount = %d", pContext->SamplesToExitCount);
            break;
        ...
        case EFFECT_CMD_SET_VOLUME:
            ...
            printf("EFFECT_CMD_SET_VOLUME left %d, right %d mode %d",
                                    ...
                                    effect_descriptor_t *pDescriptor)
{
    ...
    if (pContext == NULL || pDescriptor == NULL) {
        printf("Reverb_getDescriptor() invalid param");
        return -EINVAL;
    }
